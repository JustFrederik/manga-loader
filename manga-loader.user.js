// ==UserScript==
// @name       Manga Loader (fork)
// @namespace  https://github.com/JustFrederik/manga-loader
// @version    1.11.30
...
// -- FOOLSLIDE END
// @downloadURL https://github.com/JustFrederik/manga-loader/raw/refs/heads/master/manga-loader.user.js
// @updateURL https://update.greasyfork.org/scripts/692/Manga%20Loader%20%28unmaintained%29.meta.js
// ==/UserScript==
...
var implementations = [
  {
    ...
    img: function (ctx) {
      ...
      if (img) {
        ...
        var imgs = getEls("#content > div:nth-child(8) > img", ctx).map(
          function (page) {
            return page.src;
          },
        );
    ...
    nextchap: function (prev) {
      ...
      for (var i = 0; i != menu.length; i += 1) {
        ...
        if (img && img.title == (prev ? "Previous Chapter" : "Next Chapter")) {
          return img.parentNode.href.replace(
            /https?/,
  ...
  },
  {
    ...
    pages: function (url, num, cb, ex) {
      var imp = this;
      if (this.imgURLs[num]) cb(this.imgURLs[num], num);
      else
        ajax({
    ...
    wait: function () {
      ...
      return (
        el && el.getAttribute("src") != el.getAttribute("data-loading-img")
      );
    },
  },
  {
    ...
    numpages: function () {
      var lastPage = getEl(
        ".subnav-wrapper .controls .btn-group:last-child ul li:last-child",
      );
    ...
    nextchap: function (prev) {
      ...
      var chapters = [].slice.call(
        document.querySelectorAll(
          ".controls > div:first-child > .dropdown-menu > li a",
        ),
      );
      chapters.pop();
  ...
  },
  {
    ...
    match:
      "^https?://(mngcow|mangadoom|mangaindo|merakiscans|www\\.3asq|moonbunnycafe)\\.(co|id|info|com)/[^/]+/[0-9.]+",
    img: ".prw a > img",
    next: ".prw a",
  ...
  },
  {
    ...
    wait: function () {
      return areDefined(
        W.current_chapter_index,
        W.chapter_list,
        getEl("#top_chapter_list"),
      );
    },
  },
  {
  ...
  },
  {
    ...
    match:
      "^https?://(centraldemangas\\.org|[^\\.]+\\.com\\.br/leitura)/online/[^/]*/[0-9]*",
    img: "#manga-page",
    next: "#manga-page",
  ...
  },
  {
    ...
    nextchap: function (prev) {
      return (
        W.series_url +
        "/" +
        (parseInt(W.chapter.slice(1)) + (prev ? -1 : 1)) +
        "/1"
      );
    },
    prevchap: function () {
  ...
  },
  {
    ...
    nextchap: function (prev) {
      return (
        W.series_url +
        "chapter-" +
        (parseInt(W.chapter.match(/[0-9]+/)[0]) + (prev ? -1 : 1)) +
        "-page-1.html"
      );
    },
    prevchap: function () {
  ...
  },
  {
    ...
    nextchap: function (prev) {
      ...
      if (chnum < ps.ch.length && chnum > -1) {
        return (
          ps.action.split("?")[0] + "?ch=" + ps.ch[chnum].value + "&page=1"
        );
      } else {
        return false;
      }
    ...
    wait: function () {
      ...
      for (var i = 0; i < ps.page.length; i++) {
        this._pages.push(
          base + "?ch=" + ps.ch.value + "&page=" + ps.page[i].value,
        );
      }
  ...
  },
  {
    ...
    curpage: function () {
      return parseInt(
        getEls("#page-mainer a.btn.dropdown-toggle")[1].textContent.match(
          /[0-9]+/,
        )[0],
      );
    },
  ...
  },
  {
    ...
    nextchap: function (prev) {
      ...
      if (next)
        return location.href.replace(/\?gallery=[0-9]+/, "?gallery=" + next);
    },
    prevchap: function () {
      return this.nextchap(true);
    },
  ...
  },
  {
    ...
    nextchap: function () {
      var chapter =
        document.querySelector("#jump_chapter").selectedOptions[0]
          .previousElementSibling;
      return chapter === null
        ? false
        : this._base.replace(/[0-9]+\/$/, chapter.value);
    },
    prevchap: function () {
      var chapter =
        document.querySelector("#jump_chapter").selectedOptions[0]
  ...
  },
  {
    ...
    nextchap: function (prev) {
      var desired;
      var dropdown =
        getEls("ul.dropdown")[1] ||
        getEls("ul.uk-nav")[1] ||
        getEls("ul.dropdown-menu")[3];
      if (!dropdown) return;
      getEls("a", dropdown).forEach(function (chap, idx, arr) {
        if (location.href.indexOf(chap.href) === 0)
          desired = arr[idx + (prev ? 1 : -1)];
      });
    ...
    nextchap: function (prev) {
      ...
      if (next) {
        ...
        return location.href
          .replace(/chapter-[0-9.]+/, "chapter-" + chapter)
  ...
  },
  {
    ...
    next: function () {
      if (!this._count)
        this._count = extractInfo(this.curpage, { type: "value" });
  ...
  },
  {
    name: "kukudm",
    match:
      "^https?://(www|comic|comic2|comic3).kukudm.com/comiclist/[0-9]+/[0-9]+/[0-9]+.htm",
    img: function (ctx) {
      var script = getEl("td > script[language=javascript]", ctx);
      if (script) {
        return (
          "http://n.kukudm.com/" + script.textContent.match(/\+"([^']+)/)[1]
        );
      }
    },
    next: function (ctx) {
  ...
  },
  {
    ...
    nextchap: function (prev) {
      ...
      if (next)
        return location.origin + "/" + data[0].slice(1, -1) + "/" + next;
    },
    prevchap: function () {
      return this.nextchap(true);
    },
  ...
  },
  {
    ...
    match:
      "^https?://(spinybackmanga.com/\\?manga=[^&]+&chapter=.+|www\.titaniascans\.com/reader/.+/.+)",
    img: "#thePicLink img",
    next: "#thePicLink",
    curpage: function () {
      return W.current;
    ...
    curchap: function () {
      return parseInt(
        getEls(".selector")[1].firstChild.textContent.match(/[0-9]+/)[0],
      );
    },
    numchaps: function () {
    ...
    nextchap: function (prev) {
      var nextChap = document.scripts[2].textContent.match(
        /location.href = "([^"]+)"/,
      )[1];
      if (prev) {
        [].some.call(
          getEls(".selector .options")[1].children,
  ...
  },
  {
    ...
    next: "#page > img",
    numpages:
      "#chapter > div:nth-child(1) > div > div.uk-width-large-1-3.uk-width-medium-1-3.uk-width-small-1-1.uk-text-left.uk-text-center-small > div > div > div > ul",
    curpage:
      "#chapter > div:nth-child(1) > div > div.uk-width-large-1-3.uk-width-medium-1-3.uk-width-small-1-1.uk-text-left.uk-text-center-small > div > a.uk-button.uk-button-primary.number.uk-button-danger",
    nextchap:
      "#chapter > div:nth-child(5) > div.uk-grid.uk-grid-collapse.uk-margin-top > div.uk-width-large-1-3.uk-width-medium-1-3.uk-width-small-1-1.uk-text-left.uk-text-center-small > a",
    prevchap:
      "#chapter > div:nth-child(5) > div.uk-grid.uk-grid-collapse.uk-margin-top > div.uk-width-large-1-3.uk-width-medium-1-3.uk-width-small-1-1.uk-text-right.uk-text-center-small > a",
    pages: function (url, num, cb, ex) {
      cb(W.pages[num - 1].url, num);
    },
  },
  ...
  },
  {
    ...
    nextchap: function (prev) {
      var button = prev
        ? getEl("div.m3p > input:first-of-type")
        : getEl("div.m3p > input:last-of-type");
      return (
        button && button.attributes.onclick.value.match(/\.href='([^']+)'/)[1]
      );
    },
    prevchap: function () {
      return this.nextchap(true);
  ...
  },
  {
    ...
    nextchap: function (prev) {
      ...
      for (var i = 0; i < buttons.length; i++) {
        if (
          buttons[i].textContent.indexOf(
            prev ? "Prev Chapter" : "Next Chapter",
          ) > -1
        ) {
  ...
  },
  {
    name: "ikanman",
    match:
      "https?://(www|tw)\.(ikanman|manhuagui)\.com/comic/[0-9]+/[0-9]+\.html",
    img: "#mangaFile",
    next: function () {
      return W._next;
    },
    ...
    wait: function () {
      if (getEl("#mangaFile")) {
        ...
        W._next = location.href.replace(
          /(_p[0-9]+)?\.html.*/,
          "_p" + (ex("curpage") + 1) + ".html",
        );
  ...
  },
  {
    ...
    nextchap: function (prev) {
      return (
        location.origin +
        "/node/" +
        extractInfo("#edit-select-node", { type: "value", val: prev ? -1 : 1 })
      );
    },
    prevchap: function () {
      return this.nextchap(true);
  ...
  },
  {
    ...
    nextchap: function (prev) {
      var num = parseInt(
        extractInfo("#chapter", { type: "value", val: prev ? 1 : -1 }),
      );
      return (
        num &&
        location.href.replace(
  ...
  },
  {
    ...
    next: function () {
      return (
        location.origin + location.pathname + "?p=" + (W.SinTheme.getPage() + 1)
      );
    },
    pages: function (url, num, cb, ex) {
      cb(
        new URL(W.pageImage).origin +
          "/" +
          W.chapterPath +
          W.chapterImages[num - 1],
        num - 1,
      );
    },
    curpage: function () {
    ...
    nextchap: function () {
      return W.nextChapterData.id && W.nextChapterData.id > 0
        ? W.comicUrl + W.nextChapterData.id + ".html"
        : null;
    },
    prevchap: function () {
      return W.prevChapterData.id && W.prevChapterData.id > 0
        ? W.comicUrl + W.prevChapterData.id + ".html"
        : null;
    },
    wait: "#images > img",
  },
...
var extractInfo = function (selector, mod, context) {
  ...
  if (elem) {
    switch (elem.nodeName.toLowerCase()) {
      case "img":
        return (
          (mod.altProp && elem.getAttribute(mod.altProp)) ||
          elem.src ||
      ...
      case "select":
        switch (mod.type) {
          ...
          case "text":
            option =
              elem.options[elem.options.selectedIndex + (mod.val || 0)] || {};
...
var getViewer = function (prevChapter, nextChapter) {
  ...
  document.head.innerHTML =
    '<meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href="//maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css">';
  document.title = title;
  document.body.className = "";
  document.body.style = "";
  // navigation
  var nav =
    '<div class="ml-chap-nav">' +
    (prevChapter
      ? '<a class="ml-chap-prev" href="' + prevChapter + '">Prev Chapter</a> '
      : "") +
    '<a class="ml-exit" href="' +
    location.href +
    '" data-exit="true">Exit</a> ' +
    (nextChapter
      ? '<a class="ml-chap-next" href="' + nextChapter + '">Next Chapter</a>'
      : "") +
    "</div>";
  // message area
  ...
  // stats
  var stats =
    '<div class="ml-box ml-stats"><span title="hide stats" class="ml-stats-collapse">&gt;&gt;</span><span class="ml-stats-content"><span class="ml-stats-pages"></span> ' +
  ...
  document.body.innerHTML =
    nav + '<div class="ml-images"></div>' + nav + floatingMsg + stats;
  // add main styles
  addStyle(
    "main",
    true,
    ...
    keySettingCss,
    autoloadSettingCss,
    floatingMsgAnchorCss,
  );
  // add user styles
  var userCss = storeGet("ml-setting-css-profiles");
  var curProf = storeGet("ml-setting-css-current") || "Default";
  if (userCss && userCss.length > 0)
    userCss = userCss.filter(function (p) {
      return p.name === curProf;
    });
  userCss =
    userCss && userCss.length > 0
      ? userCss[0].css
      : storeGet("ml-setting-css") || "";
  ...
  document.addEventListener("click", function (evt) {
    if (evt.target.nodeName === "A" && evt.button !== 2) {
      var shouldReload =
        evt.target.href.indexOf("#") !== -1 &&
        evt.target.href.split("#")[0] ===
          document.location.href.split("#")[0] &&
        evt.button === 0; // fix for batoto https weirdness
    ...
    if (isMessageFloating() && UI.lastFloat === evt.target) {
      ...
      showFloatingMsg(
        [
          ...
          pageStats.curChap !== null && pageStats.numChaps !== null
            ? pageStats.curChap -
              1 +
              "/" +
              pageStats.numChaps +
              " chapters read " +
              ((((pageStats.curChap - 1) / pageStats.numChaps) * 100).toFixed(
                2,
              ) +
                "%") +
              " of series"
  ...
  UI.btnManualReload.addEventListener("click", function (evt) {
    var imgClick = function (e) {
      ...
      if (
        target.nodeName === "IMG" &&
        target.parentNode.className === "ml-images"
      ) {
        showFloatingMsg("");
        if (!target.title) {
    ...
    showFloatingMsg(
      "Left click the image you would like to reload.\nClick on the page margin to cancel.",
    );
  ...
  UI.btnSettings.addEventListener("click", function (evt) {
    if (isMessageFloating() && UI.lastFloat === evt.target) {
      ...
      settings +=
        ...
        '<textarea style="width: 300px; height: 300px;" type="text" class="ml-setting-css">' +
        prof.css +
        "</textarea><br><br>";
      // start new column
      settings += "</td><td>";
      // Keybindings
      var keyTableHtml = Object.keys(UI.keys)
        .map(function (action) {
          return (
            "<tr><td>" +
            action +
            '</td><td><input data-ignore="true" data-key="' +
            action +
            '" type="text" value="' +
            UI.keys[action] +
            '"></td></tr>'
        ...
        .join("");
      settings +=
        'Keybindings:<br><table class="ml-setting-key">' +
        keyTableHtml +
        "</table><br>";
      // Autoload
      settings +=
        'Auto-load: <input class="ml-setting-autoload" type="checkbox" ' +
        ((storeGet("mAutoload") && "checked") || "") +
        "><br><br>";
      // Load all or just N pages
      settings +=
        "# of pages to load:<br>" +
        'Type "all" to load all<br>default is 10<br>' +
        '<input class="ml-setting-loadnum" size="3" type="text" value="' +
        (storeGet("mLoadNum") || 10) +
        '" /><br><br>';
      // close grid and column
      settings += "</td></tr></table>";
      // Save button
      settings +=
        '<button class="ml-setting-save">Save</button> <button class="ml-setting-close">Close</button> <span class="ml-setting-save-flash"></span>';
      ...
      // delete css profile
      getEl(".ml-setting-delete-profile", UI.floatingMsg).onclick = function (
        e,
      ) {
        if (["Default", "New Profile..."].indexOf(prof.name) === -1) {
          if (
            confirm(
              'Are you sure you want to delete profile "' + prof.name + '"?',
            )
          ) {
      ...
      getEl(".ml-setting-css-profile", UI.floatingMsg).onchange = function (e) {
        ...
        if (prof.addNew) {
          ...
          while (
            ...
            newName = prompt(
              "Enter the name for the new profile (must be unique)",
            );
            if (!newName) {
      ...
      getEl(".ml-setting-save", UI.floatingMsg).onclick = function () {
        ...
        mLoadNum = getEl(".ml-setting-loadnum").value =
          loadnum.toLowerCase() === "all" ? "all" : parseInt(loadnum) || 10;
  ...
  var changeZoom = function (action, elem) {
    ...
    if (!lastZoom) {
      lastZoom = originalZoom = Math.round(
        (curImage.clientWidth / window.innerWidth) * 100,
      );
    }
    var zoom = lastZoom;
    if (action === "+") zoom += 5;
...
var loadManga = function (imp) {
  var ex = extractInfo.bind(imp),
    ...
    addAndLoad = function (img, next) {
      ...
      if (!next && curPage < numPages)
        throw new Error("failed to retrieve next url for page " + curPage);
      loadNextPage(next);
    },
    updateStats = function () {
      ...
      UI.statsPages.textContent =
        " " + pagesLoaded + (numPages ? "/" + numPages : "") + " loaded";
    },
    ...
    loadNextPage = function (url) {
      if (mLoadNum !== "all" && count % mLoadNum === 0) {
        if (resumeUrl) {
          ...
          resumeUrl = url;
          log(
            "waiting for user to scroll further before loading more images, loaded " +
              count +
              " pages so far, next url is " +
              resumeUrl,
          );
          return;
        }
  ...
  if (!imgUrl || (!nextUrl && curPage < numPages)) {
    log(
      "failed to retrieve " + (!imgUrl ? "image url" : "next page url"),
      "exit",
    );
  }
  ...
  nextChapter =
    nextChapter && nextChapter.trim() === location.href + "#"
      ? null
      : nextChapter;
  prevChapter =
    prevChapter && prevChapter.trim() === location.href + "#"
      ? null
      : prevChapter;
    ...
    window.addEventListener(
      ...
      throttle(function (e) {
        ...
        if (scrollBottom < UI.imageHeight * 2) {
          log(
            "user scroll nearing end, loading more images starting from " +
              resumeUrl,
          );
...
var MLoaderLoadImps = function (imps) {
  var success = imps.some(function (imp) {
    if (imp.match && new RegExp(imp.match, "i").test(pageUrl)) {
      ...
      if (urlParams.has("open-reader")) {
        waitAndLoad(imp);
      } else {
        btnLoad = createButton(
          "Load Manga",
          function (evt) {
            waitAndLoad(imp);
            this.remove();
